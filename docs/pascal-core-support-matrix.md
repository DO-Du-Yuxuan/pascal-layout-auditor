...
| door | `schema/nodes/door.ts` / `door/definition.ts` FP | hinged, double, french, folding, pocket, barn, sliding, 3 garage types; openingKind | hosted | partial | complete | complete | Pascal-equivalent physical cutout + symbols | complete/complete/complete | 2D equivalent; no editor drag handles |
