...
5. Compute one uniform scale: `min(targetWidth / cropWidth, targetDepth / cropHeight)`.
6. Center the resulting draw rectangle inside the unchanged physical footprint.
7. Render through a nested SVG whose `viewBox` is the source pixel crop. The source raster remains at its natural aspect ratio.

Irregular furniture is not stretched to fill rectangle corners. Transparent margin removal only makes its real content use the available footprint more effectively.
...
The Item inspector exposes natural dimensions, crop bbox, threshold, fallback reason, both aspect ratios, mismatch percentage, final draw dimensions, offsets, and whether uniform scaling was applied.
