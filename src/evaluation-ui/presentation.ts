...
export function evaluationFocusTargets(rule: RuleResult, nodes: Record<string, NodeData>, roomAnalysis?: RoomRegionAnalysis | null): EvaluationFocusTarget[] {
  ...
  const focusIds = rule.ruleId === "G1-023" ? [...new Set(rule.diagnostics.filter((diagnostic) => diagnostic.code === "item_outside_building_envelope").flatMap((diagnostic) => diagnostic.normalizedObjectIds.slice(0, 1)))] : ["G3-002", "G3-007", "G3-008"].includes(rule.ruleId) ? [...new Set(rule.diagnostics.flatMap((diagnostic) => diagnostic.normalizedObjectIds.slice(0, 1)))] : rule.normalizedObjectIds;
  ...
  const nodeTargets = primary.map((node, index) => {
    ...
    const relatedIds = rule.ruleId.startsWith("G3-00") ? rule.diagnostics.filter((diagnostic) => diagnostic.normalizedObjectIds.includes(node.id)).flatMap((diagnostic) => diagnostic.normalizedObjectIds.filter((id) => id !== node.id && nodes[id])).filter((id, index, all) => all.indexOf(id) === index) : hostId && nodes[hostId] ? [hostId] : [];
    return { primaryId: node.id, relatedIds, label: `${typeLabel} · ${levelName}`, levelId, levelName };
  });
...
export function designerRulePresentation(rule: RuleResult, nodes: Record<string, NodeData>, roomAnalysis?: RoomRegionAnalysis | null): DesignerRulePresentation {
  ...
  if (rule.ruleId === "G3-007") return { title: rule.status === "issue" ? `发现 ${count} 扇无法正常打开的门` : rule.status === "unable_to_determine" ? "部分门的开启关系暂时无法核验" : "平开门均可达到基本开启状态", description: rule.status === "issue" ? `${targets[0]?.label ?? "该门"}在达到基本开启角度前会碰到${targets[0]?.relatedIds.map((id) => nodes[id]?.name || nodes[id]?.type).filter(Boolean).join("、") || "障碍物"}，无法满足基本进出。` : rule.status === "unable_to_determine" ? "当前双开门只有总门宽和一组开向信息，系统不能可靠推断两片门扇各自的铰链、宽度与扫掠范围。" : rule.summary, rationale: "本项只检查达到基本开启角度之前的严重阻挡，不把极限角度的轻微接触判为问题。", recommendation: rule.status === "issue" ? "检查门的开启方向、铰链侧，以及图中橙色对象的位置。" : rule.status === "unable_to_determine" ? "为双开门补充两片门扇各自的几何关系，或在数据格式支持后重新评价。" : "无需处理。", supplemental: rule.status === "unable_to_determine" ? "这不代表已经发现双开门无法开启。" : undefined, problemCountLabel: rule.status === "issue" ? String(count) : rule.status === "unable_to_determine" ? "未确定" : "0", targets };
  if (rule.ruleId === "G3-008") return { title: rule.status === "issue" ? `发现 ${count} 组严重门扇互锁` : rule.status === "unable_to_determine" ? "严重门扇互锁暂时无法完整核验" : "未发现严重门扇互锁", description: rule.status === "issue" ? "两扇门在达到基本开启角度前会相互撞到对方关闭状态的门扇，可能导致两扇门相互锁死。" : rule.status === "unable_to_determine" ? "已检查具备可靠操作几何的单开门，但双开门缺少两片门扇的独立关系，因此不能完整排除互锁。" : rule.summary, rationale: "只有双向实际门扇阻挡才判为 G3 问题；可以顺序开关的普通相邻门只保留诊断。", recommendation: rule.status === "issue" ? "调整其中一扇门的开启方向、铰链侧或位置。" : rule.status === "unable_to_determine" ? "补充双开门每片门扇的铰链、宽度和开向后重新评价。" : "无需处理。", supplemental: rule.status === "unable_to_determine" ? "这不代表已经发现严重互锁。" : undefined, problemCountLabel: rule.status === "issue" ? String(count) : rule.status === "unable_to_determine" ? "未确定" : "0", targets };
  return {
    title: rule.ruleName,
