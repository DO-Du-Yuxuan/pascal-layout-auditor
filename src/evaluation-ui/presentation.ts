...
export function evaluationFocusTargets(rule: RuleResult, nodes: Record<string, NodeData>, roomAnalysis?: RoomRegionAnalysis | null): EvaluationFocusTarget[] {
  ...
  const nodeTargets = primary.map((node, index) => {
    ...
    const relatedIds = rule.ruleId.startsWith("G3-") || rule.ruleId === "G1-023" ? rule.diagnostics.filter((diagnostic) => diagnostic.normalizedObjectIds[0] === node.id).flatMap((diagnostic) => diagnostic.normalizedObjectIds.filter((id) => id !== node.id && nodes[id] && resolveAncestorLevelId(id, nodes).levelId === levelId)).filter((id, index, all) => all.indexOf(id) === index) : hostId && nodes[hostId] ? [hostId] : [];
...
export function designerRulePresentation(rule: RuleResult, nodes: Record<string, NodeData>, roomAnalysis?: RoomRegionAnalysis | null): DesignerRulePresentation {
  ...
  if (finalOperationRule(rule.ruleId)) {
    const copy: Record<string, { issue: string; unable: string; rationale: string; recommendation: string }> = {
      ...
      "G3-013": { issue: "可开启窗前无法正常操作", unable: "部分可开启窗的操作位置无法建立", rationale: "V0.1把所有非fixed窗都作为日常操作窗，检查窗内侧600毫米基本操作位置。", recommendation: "移动窗前家具，或核对窗户位置、宿主墙和Room归属。" },
