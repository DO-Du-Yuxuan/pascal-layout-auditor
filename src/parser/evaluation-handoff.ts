...
export function buildEvaluationHandoff(parsed: Parsed) {
  ...
  const doors = ofType("door").map((node) => { const orientation = resolveDoorOperationOrientation(node); return { ...opening(node), doorType: node.doorType ?? "hinged", hingesSide: node.hingesSide ?? null, swingDirection: node.swingDirection ?? null, rawRotationYRadians: orientation.rawRotationYRadians, placementFlipped: orientation.placementFlipped, effectiveHingesSide: orientation.effectiveHingesSide, effectiveSwingDirection: orientation.effectiveSwingDirection, swingAngleRadians: Number.isFinite(node.swingAngle) ? node.swingAngle : null, slideDirection: node.slideDirection ?? null, trackStyle: node.trackStyle ?? null, leafCount: Number.isFinite(node.leafCount) ? node.leafCount : null, garagePanelCount: Number.isFinite(node.garagePanelCount) ? node.garagePanelCount : null }; });
