...
import { finalDimensions, resolveAncestorLevelId, resolveDoorOperationOrientation, resolveItemPlanTransform, resolveItemVerticalRange, resolveWallOpeningTransform } from "../geometry/transform";
import { resolveShelfData, shelfCorners } from "../geometry/shelf";
...
export function buildEvaluationHandoff(parsed: Parsed) {
  ...
  const shelves = ofType("shelf").map((node) => { const shelf = resolveShelfData(node); return { ...base(node, nodes), style: shelf.style, dimensionsMeters: [shelf.width, shelf.height, shelf.depth], footprint: shelfCorners(node, nodes).map((corner) => [corner.x, corner.z] as Point), rows: shelf.rows, columns: shelf.columns, childItemIds: node.children ?? [] }; });
  const stairs = ofType("stair").map((node) => { const landing = stairLandingGeometry(node, nodes); return { ...base(node, nodes), stairType: node.stairType ?? "straight", fromLevelId: node.fromLevelId ?? null, toLevelId: node.toLevelId ?? null, widthMeters: Number.isFinite(node.width) ? node.width : null, totalRiseMeters: Number.isFinite(node.totalRise) ? node.totalRise : null, stepCount: Number.isFinite(node.stepCount) ? node.stepCount : null, innerRadiusMeters: Number.isFinite(node.innerRadius) ? node.innerRadius : null, sweepAngleRadians: Number.isFinite(node.sweepAngle) ? node.sweepAngle : null, footprint: stairCorners(node, nodes).map((corner) => [corner.x, corner.z] as Point), fromLandingCenter: landing ? [landing.fromCenter.x, landing.fromCenter.z] as Point : null, toLandingCenter: landing ? [landing.toCenter.x, landing.toCenter.z] as Point : null, fromLandingOutward: landing ? [landing.fromOutward.x, landing.fromOutward.z] as Point : null, toLandingOutward: landing ? [landing.toOutward.x, landing.toOutward.z] as Point : null, landingWidthMeters: landing?.width ?? null }; });
